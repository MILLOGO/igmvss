...
if(isset($_GET['type'])) {
    ...
    $nomstructure=11;
    if (!empty($tableau)) {
        echo "<option value=''>Selectionner le gestionnaire </option>";
...
//traitement pour chercher les infos du gestionnaire selectioné
...
if($idgestionnaire!=''){
    ...
    echo "<p style='color: red'> Aucun gestionnaire sélectionné </p>";
}
}
...
if(isset($_GET['idopt'])) {
    $id = $_GET['idopt'];
    if ($id !='') {
        $detailInd = Bd_GestionProjetActeur::ListerOperateurParId($id);
        $ty = 2;
        $no = 3;
        $pre = 4;
        $mail = 5;
        ...
        echo "<p style='color: red'> Aucun opérateur sélectionné </p>";
    }
}
